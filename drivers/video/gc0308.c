...
#ifdef CONFIG_VIDEO_GC0308_VGA
#  define GC0308_WIDTH   640
#  define GC0308_HEIGHT  480
#elif defined(CONFIG_VIDEO_GC0308_QVGA)
#  define GC0308_WIDTH   320
#  define GC0308_HEIGHT  240
#elif defined(CONFIG_VIDEO_GC0308_QQVGA)
#  define GC0308_WIDTH   160
#  define GC0308_HEIGHT  120
#else
#  error "No GC0308 resolution selected"
#endif
...
static const struct v4l2_frmsizeenum g_gc0308_frmsizes[] =
{
  {
...
static struct gc0308_dev_s g_gc0308_priv;
...
static int gc0308_init(struct imgsensor_s *sensor)
{
  ...
  ret = gc0308_putreglist(priv->i2c, g_gc0308_init_regs,
                           sizeof(g_gc0308_init_regs) /
                           sizeof(g_gc0308_init_regs[0]));
  if (ret < 0)
    {
  ...
  uint8_t fmt = 0;
  gc0308_getreg(priv->i2c, GC0308_REG_OUTPUT_FMT, &fmt);
  fmt = (fmt & 0xf0) | 0x06;  /* RGB565 */
  ret = gc0308_putreg(priv->i2c, GC0308_REG_OUTPUT_FMT, fmt);
  if (ret < 0)
    {
      return ret;
    }

#ifndef CONFIG_VIDEO_GC0308_VGA
  /* Non-VGA resolutions need subsample — switch to page 1 */
  ...
  uint8_t val = 0;
  gc0308_getreg(priv->i2c, GC0308_P1_REG_SUB_CTRL, &val);
  ret = gc0308_putreg(priv->i2c, GC0308_P1_REG_SUB_CTRL, val | 0x80);
  gc0308_getreg(priv->i2c, GC0308_P1_REG_SUB_EN, &val);
  ret |= gc0308_putreg(priv->i2c, GC0308_P1_REG_SUB_EN, val | 0x01);

#  if defined(CONFIG_VIDEO_GC0308_QVGA)
  ret |= gc0308_putreg(priv->i2c, GC0308_P1_REG_SUB_RATIO, 0x22);
#  elif defined(CONFIG_VIDEO_GC0308_QQVGA)
  ret |= gc0308_putreg(priv->i2c, GC0308_P1_REG_SUB_RATIO, 0x44);
#  endif
              ...
              imgsensor_interval_t *interval)
{
  ...
  if (datafmts[IMGSENSOR_FMT_MAIN].width != GC0308_WIDTH ||
      datafmts[IMGSENSOR_FMT_MAIN].height != GC0308_HEIGHT)
    {
      return -EINVAL;
    }
...
{
  struct gc0308_dev_s *priv = &g_gc0308_priv;
  ...
  priv->sensor.fmtdescs_num  = sizeof(g_gc0308_fmtdescs) /
                                sizeof(g_gc0308_fmtdescs[0]);
  priv->sensor.frmsizes      = g_gc0308_frmsizes;
  priv->sensor.frmsizes_num  = sizeof(g_gc0308_frmsizes) /
                                sizeof(g_gc0308_frmsizes[0]);
  priv->sensor.frmintervals      = g_gc0308_frmintervals;
  priv->sensor.frmintervals_num  = sizeof(g_gc0308_frmintervals) /
                                    sizeof(g_gc0308_frmintervals[0]);
